...
class StorageService(KombuMixin):
    ...
    async def create(self, app_data: AppData):
        ...
        self.logger.debug(f'Increase task: {app_data.app_id.task_id} item count.')
    ...
    async def _exporter(self, job_id: int) -> BaseExporter:
        ...
        if server:
            ...
            if exporter_cls:
                ...
                raise Exception(f'Exporter schema: {server.type} is not support. Please implementation')
        else:
    ...
    def callback(self, func: Callable, storage_id: int, body: Dict, message: Message):
        ...
        try:
            ...
            self.logger.error(f'Consume data error. {e}')
        # 如果没有异常，则更新状态，同时 ack
        else:
    ...
    async def consume_task(
        ...
        :return:
        """
        self.logger.debug(f'Start storage consume task: {app_id}')
        ...
        self.logger.debug(f'storage task : {app_id} {state}')
        await run_in_executor(
        ...
        if str(app_id) not in self.__storage_tasks:
            ...
            if running_storage:
                for storage in running_storage:
                    self.logger.debug(f'Because storage: {storage.id} already stop, update db state')
                    await run_in_executor(
                        storage_dao.update_by_id,
                        pk=storage.id,
                        obj_in=StorageUpdate(state=States.Stopped, detail='storage task already stop, update db state')
                    )
            return True
    ...
    def _clean_storage_task(self, app_id: AppId) -> bool:
        """
        如果正常停止 fut 返回 true
        :param app_id:
        :return:
    ...
    async def stop(self, app_id: AppId) -> str:
        if str(app_id) in self.__storage_tasks:
            self._clean_storage_task(app_id)
            self.logger.debug(f'Stopping storage task: {app_id}')
    ...
    async def server_stop(self):
        ...
        if not self.server_running:
            self.logger.debug(f'Server stop, clean storage task.')
