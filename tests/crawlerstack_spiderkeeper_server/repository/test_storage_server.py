...
async def test_get_snapshot_server_from_job_id(init_job, repo, session, job_id, exist):
    """Test get a object."""
    if exist:
        exist_obj = await session.scalar(
            select(Job).filter(Job.id == job_id).options(selectinload(Job.snapshot_server)))
        obj = await repo.get_snapshot_server_from_job_id(exist_obj.id)
        assert obj
        assert exist_obj.storage_server.id == obj.id
    else:
        with pytest.raises(ObjectDoesNotExist):
